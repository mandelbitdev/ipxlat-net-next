...
ipxlat_capture_pkts "$NS4" "icmp and icmp[0] == 3 and icmp[1] == 3" 1 3 \
	ip netns exec "$NS4" bash -c "echo x > /dev/udp/$IPXLAT_V4_REMOTE/9 || true"
...
ipxlat_capture_pkts "$NS6" "icmp6 and ip6[40] == 1 and ip6[41] == 4" 1 3 \
	ip netns exec "$NS6" bash -c "echo x > /dev/udp/$IPXLAT_V6_NS4/9 || true"
...
ipxlat_capture_pkts "$NS4" "icmp and icmp[0] == 3 and icmp[1] == 4" 1 3 \
	...
	"ping -M \"do\" -s 1300 -c 1 -W 1 \"$IPXLAT_V4_REMOTE\" >/dev/null 2>&1 || test \$? -eq 1"
