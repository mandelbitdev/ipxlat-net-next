...
RET=0
ip netns exec "$NS4" ping -c 2 -W 2 "$IPXLAT_V4_REMOTE"
check_err $? "ping 4->6 failed"
log_test "icmp-info 4->6"

RET=0
ip netns exec "$NS6" ping -6 -c 2 -W 2 -I "$IPXLAT_V6_NS6_SRC" "$IPXLAT_V6_NS4"
...
RET=0
ipxlat_run_iperf "$NS4" "$NS6" "$IPXLAT_V6_NS4" 5201 -B "$IPXLAT_V6_NS6_SRC" -n 256K
