...
ipxlat_configure_topology()
{
	...
	ip netns exec "$NS4" ip addr add "$IPXLAT_NS4_ADDR/24" dev "$IPXLAT_VETH4_NS"
	ip netns exec "$NS4" ip link set "$IPXLAT_VETH4_NS" up
	ip netns exec "$NS4" ip route add default via "$IPXLAT_HOST4_ADDR"

	ip netns exec "$NS6" ip -6 addr add "$IPXLAT_NS6_ADDR/64" dev "$IPXLAT_VETH6_NS"
	ip netns exec "$NS6" ip -6 addr add "$IPXLAT_V6_REMOTE/128" dev "$IPXLAT_VETH6_NS"
	ip netns exec "$NS6" ip -6 addr add "$IPXLAT_V6_NS6_SRC/128" dev "$IPXLAT_VETH6_NS"
	...
	ip -6 route replace "$IPXLAT_XLAT_PREFIX6/$IPXLAT_XLAT_PREFIX6_LEN" dev "$IPXLAT_VETH6_HOST"
	# 6->4 steering rule
