...
RET=0
ipxl_run_iperf "$NS6" "$NS4" "$IPXL_V4_REMOTE" 5301 -n 8M
...
RET=0
ipxl_run_iperf "$NS6" "$NS4" "$IPXL_V4_REMOTE" 5302 -u -b 20M -t 2 -l 1400
check_err $? "large udp 4->6 failed"
log_test "large udp 4->6"

# Exercise large TCP flow on 6->4 path to cover fragmentation-sensitive translation
RET=0
ipxl_run_iperf "$NS4" "$NS6" "$IPXL_V6_NS4" 5303 -B "$IPXL_V6_NS6_SRC" -n 8M
check_err $? "large tcp 6->4 failed"
log_test "large tcp 6->4"

# Exercise large UDP flow on 6->4 path to cover fragmentation-sensitive translation
RET=0
ipxl_run_iperf "$NS4" "$NS6" "$IPXL_V6_NS4" 5304 \
	-B "$IPXL_V6_NS6_SRC" -u -b 20M -t 2 -l 1400
...
# Echo seen in NS6)
RET=0
ipxl_capture_pkts "$NS6" "icmp6 and ip6[40] == 128" 0 5 \
	ip netns exec "$NS4" bash -c \
	"ping -M \"dont\" -s 2000 -c 1 -W 1 \"$IPXL_V4_REMOTE\" >/dev/null 2>&1 || test \$? -eq 1"
...
RET=0
ipxl_capture_pkts "$NS4" "icmp and icmp[0] == 8" 0 5 \
	ip netns exec "$NS6" bash -c \
	"ping -6 -s 2000 -c 1 -W 1 -I \"$IPXL_V6_NS6_SRC\" \"$IPXL_V6_NS4\" >/dev/null 2>&1 || test \$? -eq 1"
