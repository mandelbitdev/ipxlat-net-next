...
int ipxl_46_outer_tcp(struct sk_buff *skb, const struct iphdr *in4)
{
	...
	tcp_old.check = 0;
	tcp_new->check = 0;
	tcp_new->check = ipxl_46_update_csum(csum16, in4, &tcp_old, iph6,
...
int ipxl_46_outer_udp(struct sk_buff *skb, const struct iphdr *in4)
{
	...
	if (unlikely(udp_new->check == 0)) {
		...
		udp_new->check =
			ipxl_l4_csum_ipv6(&iph6->saddr, &iph6->daddr, skb,
	...
	udp_old.check = 0;
	udp_new->check = 0;
	udp_new->check = ipxl_46_update_csum(csum16, in4, &udp_old, iph6,
...
int ipxl_64_outer_tcp(struct sk_buff *skb, const struct ipv6hdr *in6)
{
	...
	tcp_old.check = 0;
	tcp_new->check = 0;
	tcp_new->check = ipxl_64_update_csum(csum16, in6, &tcp_old,
...
int ipxl_64_outer_udp(struct sk_buff *skb, const struct ipv6hdr *in6)
{
	...
	udp_old.check = 0;
	udp_new->check = 0;
	udp_new->check = ipxl_64_update_csum(csum16, in6, &udp_old,
