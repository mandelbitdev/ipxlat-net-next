					     ...
					     netdevice_tracker *tracker)
{
	...
	if (!ipxlat_dev_is_valid(dev)) {
		...
		NL_SET_BAD_ATTR(info->extack, info->attrs[IPXLAT_A_DEV_IFINDEX]);
		return ERR_PTR(-EINVAL);
	}
		     ...
		     struct genl_info *info)
{
	...
	ipxlat = ipxlat_get_from_attrs(genl_info_net(info), info, &ctx->tracker);
	if (IS_ERR(ipxlat))
			    ...
			    const u32 seq, int flags)
{
	...
	xlat_prefix6 = ipxlat->cfg.xlat_prefix6;
	lowest_ipv6_mtu = ipxlat->cfg.lowest_ipv6_mtu;
	...
	if (!net_eq(src_net, dev_net(ipxlat->dev))) {
		id = peernet2id_alloc(src_net, dev_net(ipxlat->dev), GFP_ATOMIC);
		if (id < 0) {
			ret = id;
			goto err;
		}
...
static int ipxlat_nl_parse_xlat_prefix6(struct nlattr *attr, struct ipv6_prefix *xlat_prefix6,
			       struct netlink_ext_ack *extack)
{
	...
	if (attrs_pool[IPXLAT_A_POOL_PREFIX_LEN])
		new_xlat_prefix6.len = nla_get_u8(attrs_pool[IPXLAT_A_POOL_PREFIX_LEN]);
	...
	if (ret) {
		if (attrs_pool[IPXLAT_A_POOL_PREFIX_LEN])
			...
			NL_SET_BAD_ATTR(extack, attrs_pool[IPXLAT_A_POOL_PREFIX]);
		return ret;
	}
...
int ipxlat_nl_dev_set_doit(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (!attrs[IPXLAT_A_CFG_XLAT_PREFIX6] && !attrs[IPXLAT_A_CFG_LOWEST_IPV6_MTU]) {
	...
	if (attrs[IPXLAT_A_CFG_XLAT_PREFIX6]) {
		xlat_prefix6 = ctx->ipxlat->cfg.xlat_prefix6;
		ret = ipxlat_nl_parse_xlat_prefix6(attrs[IPXLAT_A_CFG_XLAT_PREFIX6], &xlat_prefix6,
					  info->extack);
		if (ret)
			goto out_unlock;
	}
	...
	if (attrs[IPXLAT_A_CFG_XLAT_PREFIX6])
		ctx->ipxlat->cfg.xlat_prefix6 = xlat_prefix6;
	if (attrs[IPXLAT_A_CFG_LOWEST_IPV6_MTU]) {
		lowest_ipv6_mtu =
			nla_get_u32(attrs[IPXLAT_A_CFG_LOWEST_IPV6_MTU]);
		WRITE_ONCE(ctx->ipxlat->cfg.lowest_ipv6_mtu, lowest_ipv6_mtu);
	}
