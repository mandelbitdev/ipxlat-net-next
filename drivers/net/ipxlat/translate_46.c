			    ...
			    u8 l4_proto)
{
	...
	fh6->reserved = 0;
	fh6->frag_off = ipxlat_build_frag6_offset(ipxlat_get_frag4_offset(hdr4),
				  ...
				  const struct iphdr *in4)
{
	...
	ipxlat_46_convert_addr(&ipxlat->cfg.xlat_prefix6, in4->saddr, &fl6.saddr);
	ipxlat_46_convert_addr(&ipxlat->cfg.xlat_prefix6, in4->daddr, &fl6.daddr);
...
int ipxlat_46_plan_prefrag(struct ipxlat_priv *ipxlat, struct sk_buff *skb)
{
	...
	threshold6 = min(pmtu6, READ_ONCE(ipxlat->cfg.lowest_ipv6_mtu));
...
int ipxlat_46_translate(struct ipxlat_priv *ipxlat, struct sk_buff *skb)
{
	...
	iph6 = ipv6_hdr(skb);
	ipxlat_46_build_l3(iph6, &outer4, skb->len - sizeof(*iph6), out_l4_proto,
			 outer4.ttl - 1);

	/* translate IPv4 endpoints into IPv6 addresses using xlat_prefix6 prefix */
	ipxlat_46_convert_addrs(&ipxlat->cfg, &outer4, iph6);
...
out:
	...
	return ipxlat_finalize_offload(skb, in_l4_proto, has_frag, SKB_GSO_TCPV4,
				     SKB_GSO_TCPV6);
}
